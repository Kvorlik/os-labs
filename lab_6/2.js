// Умножение элемента матрицы на число
...
function TransposeMatr(A,M){
	...
	for (var i=0; i<M; i++){
		Znach[i] = new Array(M);
		for (var j=0; j<M; j++) Znach[i][j] = A[j][i];
	}
	return Znach;
}

// Поиск мирора для вычисления определителя
function MinorMatr(Mass,Num,t){
	Znach = new Array(Num);
	for (var i=0; i<Num; i++) Znach[i] = new Array(Num);
	for (var i=0; i<Num; i++){
		var ElemMinorMatr = "";
		for (var j=0; j<Num; j++){
			if (j<t) Znach[i][j] = Mass[i+1][j];
			else Znach[i][j] = Mass[i+1][j+1];
...
function DeterminantMatr(A,M){
	...
	if (M==2) return A[0][0] * A[1][1] - A[0][1] * A[1][0];
		...
		for(var j=0; j<M; j++){			
			...
			if (j%2==0) k+=A[0][j]*DeterminantMatr(Znach,M-1);
			else k-=A[0][j]*DeterminantMatr(Znach,M-1);
		}
		return k;
	}
	return 0;
...
function PrintMatrNxN(Mass){
	...
	for(var i = 0; i < N; i++) PrintStr = PrintStr+TekStr[i]+"\n";
...
function CreateMatrNxN(N){
	...
	for(var i = 0; i < N; i++) NewMatr[i] = new Array(N);
	for(var i = 0; i < N; i++){
		for(var j = 0; j < N; j++){
			if (i == j) NewMatr[i][j] = 1;
			else{
				if (i > j) NewMatr[i][j] = 4;
				else NewMatr[i][j] = 7;
			}
		}
	}
...
function SumStModElemMatr(Mass){
	for(var i = 0; i < N; i++){
		var SumElem = 0;
		if((i+1)%2 == 1){
			for(var j = 0; j < N; j++) SumElem+= Mass[i][j];
			WScript.Echo("Сумма элементов строки ",i+1," : ",SumElem);
		}
	}
}
...
function MulSbModElemMatr(Mass){
	for(var j = 0; j < N; j++){
		...
		if((j+1)%2 == 1){
			for(var i = 0; i < N; i++){
				if(Mass[i][j]%2 == 0) MulElem *= Mass[i][j];
			}
			if(MulElem != 1) WScript.Echo("Произведение четных элементов столбца",j+1,":",MulElem);
			else WScript.Echo("В столбце",j+1,"нет четных элементов");
		}
	}
}
...
function OriginalMatrMulSortMatr(IshodMatr,SortMatr){
	...
	for(var i = 0; i < N; i++) NewMulMatr[i] = new Array(N);
	for(var i = 0; i < N; i++){
		for(var j = 0; j < N; j++){
			NewMulMatr[i][j] = 0;
			for(var k = 0; k < N; k++) NewMulMatr[i][j] += IshodMatr[i][k]*SortMatr[k][j];
		}
	}	
	...
	var ResLine = "";
	for(var i = 0; i < N; i++){
		for(var j = 0; j < N; j++) ResLine += " " + NewMulMatr[i][j];
	}
